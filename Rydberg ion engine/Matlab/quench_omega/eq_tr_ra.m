...
%quench parameters
big_omega = quench(t, 2*pi, 2.0, 0.0);
delta_omega = quench(t + pi/2.0, 2*pi, 1.0, 0.0);

omega_q = omega + (delta_omega.^2.0)./(3.0*omega);
...
dydt(3,:) = (-c*omega_q.*x + k1.*nr.^2.0 - 2.0*k2.*x.*nr.^2.0)./gamma;
dydt(4,:) = (-c*omega_q.*xp_px + sqrt(8.0)*k1.*p.*nr.^2 - 4.0*k2.*xp_px.*nr.^2.0)./gamma;
dydt(5,:) = (2.0*(-c*(omega.*x2 - omega_q.*p2) + k1.*x.*nr.^2 - 4.0*k2.*x2.*nr.^2))./gamma; 
